...
۱۴۰۵/۰۶/۱۳، بعد از خاموش‌کردن پروکسی آروان:

```
baker.molido.ir  →  37.32.21.125
```

درست است. (پیش از این به anycast ابرآروان می‌رفت — `185.143.233.238` و
`185.143.234.238` — و با آن حالت، اثبات از راه فایل به سرور ما نمی‌رسید.)

بازآزمودنش هر وقت لازم شد:

```bash
dig +short baker.molido.ir     # باید فقط 37.32.21.125 بدهد
```

### پیش‌نیاز ۲ — سرور باید از بیرونِ ایران قابل دسترس باشد

**این‌جا کار خراب می‌شود و پیامش گمراه‌کننده است.** سرورهای اعتبارسنجی
Let's Encrypt در آمریکا و اروپا هستند و از همان‌جا `http://baker.molido.ir/
.well-known/acme-challenge/...` را می‌گیرند. اگر روی سرور فایروالی هست که
ترافیک خارجی را می‌بندد — یا دیتاسنتر خودش می‌بندد — درخواست هرگز نمی‌رسد و
certbot می‌گوید «Timeout during connect»، که دقیقاً شبیه «DNS غلط است» یا
«nginx بالا نیست» خوانده می‌شود، در حالی که هر دو سالم‌اند.

> از داخل این محیط تأیید نشد: هم `baker.molido.ir` و هم مستقیم
> `37.32.21.125` با خطای پروکسیِ خودِ محیط برمی‌گردند. **این نه یعنی سرور
> خواب است و نه یعنی بیدار است** — فقط یعنی از این‌جا دیده نمی‌شود.

آزمودنش قبل از اجرای certbot، که ارزان است و جلوی سردرگمی را می‌گیرد.
روی سرور:

```bash
sudo mkdir -p /var/www/html/.well-known/acme-challenge
echo it-works | sudo tee /var/www/html/.well-known/acme-challenge/probe
```

و بعد از یک شبکهٔ **خارج از ایران** (یا هر سرویس آنلاینِ «fetch this URL»):

```
http://baker.molido.ir/.well-known/acme-challenge/probe
```

اگر `it-works` برنگشت، certbot هم نمی‌تواند. راه‌ها:

- پورت ۸۰ را برای همه باز کنید (`sudo ufw allow 80/tcp`)، دست‌کم تا وقتی
  گواهی صادر و هر ۶۰ روز تمدید شود؛
- یا اثبات را از راه DNS بگیرید، که اصلاً به رسیدنِ ترافیک به سرور نیاز
  ندارد:

  ```bash
  sudo certbot certonly --manual --preferred-challenges dns -d baker.molido.ir
  ```

  certbot یک رکورد `TXT` روی `_acme-challenge.baker.molido.ir` می‌خواهد.
  کار می‌کند، ولی **دستی است و هر ۶۰ روز تکرار می‌شود** — و تمدیدی که
  یادش می‌رود، گواهی‌ای است که یک روز صبح منقضی شده. اگر این راه را
  رفتید، یک یادآور تقویمی بگذارید.

پاک‌کردن فایل آزمایش بعدش:

```bash
sudo rm /var/www/html/.well-known/acme-challenge/probe
```

### همهٔ این‌ها در یک دستور

مرحله‌های ۲ و ۳ و آزمودنشان را `scripts/enable-https.sh` انجام می‌دهد، با
محافظت‌هایی که این راهنما به زبان می‌گوید و یک آدم خسته ممکن است رد کند:

```bash
# چند روز قبل، فقط برای اطمینان از رسیدن به پورت ۸۰:
sudo PROBE_ONLY=1 ./scripts/enable-https.sh

# و روزی که می‌خواهید روشنش کنید:
sudo ./scripts/enable-https.sh
```

certbot را قبل از اثبات اینکه مسیر `/.well-known/` سرو می‌شود صدا نمی‌زند،
nginx را روی کانفیگی که تست نکرده بارگذاری نمی‌کند، و **اگر پورت ۸۰۰۰ بعدش
جواب ندهد همه چیز را برمی‌گرداند** — آن یکی تنها موردی است که مغازه را
می‌خواباند، چون گوشی‌ها هنوز با IP حرف می‌زنند. ۲۴ تست دارد.

مرحلهٔ ۱ (زیر) هنوز دستی است چون یک بار انجام می‌شود، و مرحله‌های ۴ و ۵
عمداً بیرون مانده‌اند: هر دو هم‌زمان روی همهٔ گوشی‌ها اثر می‌گذارند و باید
تصمیم باشند، نه دنبالهٔ یک اسکریپت.
