...
**از این به بعد `scripts/deploy.sh` خودش این کار را می‌کند** (قدم ۵/۷):
اگر فایل مخزن با فایل سرور فرق داشت، از سرور پشتیبان می‌گیرد
(`/root/bakery-nginx-<تاریخ>/`)، نسخهٔ مخزن را می‌گذارد، `nginx -t` می‌زند،
و اگر رد شد عیناً برمی‌گرداند. اولین استقرار بعد از این تغییر، مسیر
`acme-challenge` را که روی سرور جا افتاده بود نصب می‌کند — بعدش یک بار
`sudo certbot renew --dry-run` بزنید تا ثابت شود تمدید گواهی کار می‌کند.

اگر خواستید با دست انجامش دهید، همان روش قبلی:
