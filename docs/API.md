...
`device_name` اختیاری است (حداکثر ۶۰ نویسه). نامی است که در فهرست
`GET /devices` کنار همین نشست نوشته می‌شود. اگر نیاید، نشست `mobile-app`
نام می‌گیرد — که یعنی فهرست چند ردیف هم‌نام می‌شود و کسی که گوشی‌اش گم
شده نمی‌داند کدام را ببندد.

**بدنه:**

```json
{
  "login": "dough@bakery.test",
...
گوشی‌هایی که با حساب همین کاربر وارد شده‌اند. فقط نشست‌های خودِ درخواست‌کننده
— نه کسی دیگر.
...
`is_current` را سرور می‌گوید، نه اپ: اپ از روی ردیف‌ها نمی‌تواند تشخیص
دهد کدام نشستِ خودش است، و حدس‌زدنش یعنی بستن نشست اشتباه.

`last_used_at` برای نشستی که تازه باز شده و هنوز چیزی از آن خواسته نشده
`null` است.

---

### `DELETE /devices/{token}` 🔒 همه

یک دستگاه را خارج می‌کند. `token` شناسهٔ ردیف در فهرست بالاست.

**پاسخ ۲۰۰:** `{ "signed_self_out": true }` وقتی همان گوشیِ درخواست‌کننده
بسته شده — اپ باید توکنش را پاک کند و به صفحهٔ ورود برگردد.

**خطاها:** `404` شناسه‌ای که به نشستِ این کاربر تعلق ندارد. عمداً ۴۰۴ و نه
۴۰۳: شناسهٔ یک حساب دیگر نباید بتواند شیفت کسی را تمام کند.

---

### `DELETE /devices/others` 🔒 همه

همهٔ دستگاه‌ها به‌جز همین گوشی.

**پاسخ ۲۰۰:** `{ "closed": 2 }`

> این دکمه‌ای است که کسی واقعاً فشار می‌دهد: آدمی که تازه فهمیده گوشی‌اش
> نیست، نمی‌تواند بگوید کدام ردیف است.
...
| GET | `/devices` | احراز هویت | همه |
| DELETE | `/devices/others` | احراز هویت | همه |
| DELETE | `/devices/{token}` | احراز هویت | همه |
