...
> **این سند تاریخی است.** سرور امروز نانوایی `37.32.21.125` است — نه هیچ
> آدرسی که در ادامه می‌بینید. آدرس‌های داخل این متن مربوط به انتقال‌های
> گذشته‌اند و **هیچ‌کدام دیگر بخشی از این پروژه نیستند**:
>
> - `185.97.119.91` — سرور اول، کنار گذاشته شد
> - `194.5.176.140` — سرور دوم، کنار گذاشته شد
>
> اگر جایی در پروژه به این دو برخوردید، کهنه است و باید پاک شود.
>
> آنچه نگه داشته شده **ترتیب کار** است، نه آدرس‌ها: انتقال بعدی هم همین
> مراحل را دارد، فقط با آدرس تازه. برای همین در دستورهای زیر به‌جای آدرس
> واقعی `SERVER-GHADIMI` و `SERVER-JADID` نوشته شده — دستوری که آدرس
> مرده در خود دارد، روزی کپی می‌شود.

این سند ترتیبی را می‌نویسد که در آن هیچ گوشی‌ای بی‌سرور نمی‌ماند.
...
```bash
scp ~/bakery-*.sql ~/bakery-storage-*.tar.gz root@SERVER-JADID:~/
```
...
```
APP_URL=http://SERVER-JADID:8000
```
...
```bash
curl -s http://SERVER-JADID:8000/api/v1/health
```
...
`http://SERVER-JADID:8000/api/v1` بگذارید
...
```json
{
  "api_base_url": "http://SERVER-JADID:8000/api/v1",
  "fallback_urls": ["http://SERVER-GHADIMI:8000/api/v1"]
}
```
